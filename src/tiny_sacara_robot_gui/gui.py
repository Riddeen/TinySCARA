...
from robot_matplotlib import RobotPlot
...
class MainWindow(tk.Frame):
    def __init__(self, parent: tk.Tk, theme: str="light"):
        tk.Frame.__init__(self, parent)
        self._parent = parent
        self._multiplier = 1
        self._robot_plot = RobotPlot()
    ...
    def draw_main_gui(self):
        ...
        #Matplotlib Graph
        self.chart = FigureCanvasTkAgg(self._robot_plot.fig, self)
        self.chart.get_tk_widget().pack(side=tk.RIGHT)

    def group_control_block(self, parent, title: str, block_label_list: list[str]):
        s = ttk.Style()
        s.configure("TLabelframe.Label", font=self.font_medium)
        self.group_control_block_frame = ttk.Labelframe(parent, text=title) # style="TLabelframe"
    ...
    def update_robot_sim(self, value=0):
        self._robot_plot.update_pose([value,0,0])
